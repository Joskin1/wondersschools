...
<head>
    ...
    <title>{{ $title ?? $site->get('seo_title') ?? $site->get('school_name', config('app.name')) }}</title>
    @if($site->get('seo_description'))
    <meta name="description" content="{{ $site->get('seo_description') }}">
    @endif
    @if($site->get('seo_og_image'))
    <meta property="og:image" content="{{ Storage::url($site->get('seo_og_image')) }}">
    @endif
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-20 items-center">
                    ...
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ route('home') }}">
                            @if($site->get('site_logo'))
                                <img src="{{ Storage::url($site->get('site_logo')) }}" alt="{{ $site->get('school_name') }} Logo" class="h-12 w-auto">
                            @else
                                <span class="font-bold text-2xl tracking-wider">{{ $site->get('school_name', config('app.name')) }}</span>
                            @endif
                        </a>
                    </div>
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div>
                        <div class="mb-4">
                            @if($site->get('site_logo'))
                                <img src="{{ Storage::url($site->get('site_logo')) }}" alt="{{ $site->get('school_name') }} Logo" class="h-8 w-auto">
                            @else
                                <h3 class="text-xl font-bold text-lime-green">{{ $site->get('school_name', config('app.name')) }}</h3>
                            @endif
                        </div>
                        <p class="text-gray-400 text-sm leading-relaxed">
                            {{ $site->get('footer_description') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-2 text-sm text-gray-400">
                            @if($site->get('school_address'))
                            <li class="flex items-start">
                                <svg class="h-5 w-5 mr-2 text-lime-green flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                </svg>
                                <span>{{ $site->get('school_address') }}</span>
                            </li>
                            @endif
                            @if($site->get('school_phone'))
                            <li class="flex items-center">
                                <svg class="h-5 w-5 mr-2 text-lime-green flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                </svg>
                                <span>{{ $site->get('school_phone') }}</span>
                            </li>
                            @endif
                            @if($site->get('school_email'))
                            <li class="flex items-center">
                                <svg class="h-5 w-5 mr-2 text-lime-green flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                </svg>
                                <span>{{ $site->get('school_email') }}</span>
                            </li>
                            @endif
                        </ul>
                    </div>
                ...
                <div class="mt-8 pt-8 border-t border-gray-800 text-center text-sm text-gray-500">
                    &copy; {{ date('Y') }} {{ $site->get('school_name', config('app.name')) }}. All rights reserved.
                </div>
            </div>
        </footer>
    </div>
