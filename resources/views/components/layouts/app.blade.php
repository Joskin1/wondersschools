...
<head>
    ...
    <title>{{ $title ?? \App\Services\FrontendLibrary::get('site_name', 'Wonders Kiddies Foundation Schools') }}</title>
                    ...
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ route('home') }}">
                            @if(\App\Services\FrontendLibrary::get('site_logo'))
                                <img src="{{ Storage::url(\App\Services\FrontendLibrary::get('site_logo')) }}" alt="{{ \App\Services\FrontendLibrary::get('site_name', 'WKFS') }} Logo" class="h-12 w-auto">
                            @else
                                <span class="font-bold text-2xl tracking-wider">{{ \App\Services\FrontendLibrary::get('site_name', 'WKFS') }}</span>
                            @endif
                        </a>
                    </div>
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div>
                        <div class="mb-4">
                             @if(\App\Services\FrontendLibrary::get('site_logo'))
                                <img src="{{ Storage::url(\App\Services\FrontendLibrary::get('site_logo')) }}" alt="{{ \App\Services\FrontendLibrary::get('site_name', 'WKFS') }} Logo" class="h-8 w-auto">
                            @else
                                <h3 class="text-xl font-bold text-lime-green">{{ \App\Services\FrontendLibrary::get('site_name', 'WKFS') }}</h3>
                            @endif
                        </div>
                        <p class="text-gray-400 text-sm leading-relaxed">
                            {{ \App\Services\FrontendLibrary::get('footer_description', 'Wonders Kiddies Foundation Schools is dedicated to providing a nurturing and stimulating environment for children to learn and grow.') }}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul class="space-y-2 text-sm text-gray-400">
                            <li class="flex items-start">
                                ...
                                <span>{{ \App\Services\FrontendLibrary::get('school_address', '123 School Lane, City, Country') }}</span>
                            </li>
                            <li class="flex items-center">
                                <svg class="h-5 w-5 mr-2 text-lime-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                </svg>
                                <span>{{ \App\Services\FrontendLibrary::get('school_phone', '+123 456 7890') }}</span>
                            </li>
                            <li class="flex items-center">
                                <svg class="h-5 w-5 mr-2 text-lime-green" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                </svg>
                                <span>{{ \App\Services\FrontendLibrary::get('school_email', 'info@wkfs.com') }}</span>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="mt-8 pt-8 border-t border-gray-800 text-center text-sm text-gray-500">
                    &copy; {{ date('Y') }} {{ \App\Services\FrontendLibrary::get('site_name', 'Wonders Kiddies Foundation Schools') }}. All rights reserved.
                </div>
            </div>
        </footer>
    </div>
