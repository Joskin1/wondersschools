<div>
    ...
    <div class="relative bg-dark-green text-white overflow-hidden" x-data="{
        images: {{ json_encode($site->get('hero_images', [])) }},
        active: 0,
        init() {
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-32">
            <div class="lg:w-2/3">
                <span class="block text-lime-green font-bold tracking-wide uppercase text-sm mb-2">{{ $site->get('hero_tagline') }}</span>
                <h1 class="text-4xl lg:text-6xl font-extrabold tracking-tight leading-tight mb-6">
                    {{ $site->get('hero_heading') }}
                </h1>
                <p class="text-xl text-gray-200 mb-8 max-w-2xl leading-relaxed">
                    {{ $site->get('hero_description') }}
                </p>
                <div class="flex flex-col sm:flex-row gap-4">
                    <a href="{{ route('academics') }}" class="inline-flex items-center justify-center px-8 py-4 border border-transparent text-base font-bold rounded-md text-dark-green bg-lime-green hover:bg-opacity-90 transition duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                        {{ $site->get('hero_cta_primary_text', 'Explore Our Curriculum') }}
                    ...
                    <a href="{{ route('contact') }}" class="inline-flex items-center justify-center px-8 py-4 border-2 border-white text-base font-bold rounded-md text-white hover:bg-white hover:text-dark-green transition duration-300">
                        {{ $site->get('hero_cta_secondary_text', 'Book a Tour') }}
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="bg-lime-green py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            @php
                ...
                $trustItems = $site->get('trust_items', []);
                $trustCount = max(count($trustItems), 1);
            @endphp
            <div class="grid grid-cols-2 md:grid-cols-{{ min($trustCount, 4) }} gap-4 text-center md:text-left">
                @foreach($trustItems as $item)
                <div class="flex items-center justify-center md:justify-start space-x-2">
                    <svg class="w-6 h-6 text-dark-green" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        {!! $trustIcons[$loop->index % count($trustIcons)] !!}
                    </svg>
                    <span class="font-bold text-dark-green text-sm sm:text-base">{{ $item['label'] }}</span>
                </div>
                @endforeach
            </div>
        </div>
    ...
    <div class="py-20 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-3xl md:text-4xl font-extrabold text-gray-900 mb-4">{{ $site->get('why_us_heading') }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">{{ $site->get('why_us_subheading') }}</p>
            </div>

            @php
                $cardStyles = [
                    ['border' => 'border-lime-green', 'iconBg' => 'bg-lime-green/20', 'iconColor' => 'text-dark-green', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>'],
                    ['border' => 'border-light-blue', 'iconBg' => 'bg-light-blue/20', 'iconColor' => 'text-light-blue', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>'],
                    ['border' => 'border-brand-red', 'iconBg' => 'bg-brand-red/20', 'iconColor' => 'text-brand-red', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>'],
                ];
            @endphp
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($site->get('bento_cards', []) as $card)
                @php $cs = $cardStyles[$loop->index % count($cardStyles)]; @endphp
                <div class="bg-white p-8 rounded-2xl shadow-lg hover:shadow-xl transition duration-300 border-b-4 {{ $cs['border'] }}">
                    <div class="w-14 h-14 {{ $cs['iconBg'] }} rounded-full flex items-center justify-center mb-6 {{ $cs['iconColor'] }}">
    ...
    <div class="bg-dark-green py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
                @foreach($site->get('stats', []) as $stat)
                <div>
                    <div class="text-4xl md:text-5xl font-extrabold text-lime-green mb-2">{{ $stat['value'] }}</div>
    ...
    <div class="bg-dark-green py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-3xl md:text-4xl font-extrabold text-white mb-8">
                Ready for the {{ $site->get('school_name', config('app.name')) }} Experience?
            </h2>
            <div class="flex flex-col sm:flex-row justify-center gap-4">
                ...
                @if($site->get('social_whatsapp'))
                <a href="https://wa.me/{{ str_replace(['+', ' '], '', $site->get('social_whatsapp')) }}" target="_blank" class="inline-flex items-center justify-center px-8 py-4 border border-transparent text-lg font-bold rounded-md text-white bg-green-600 hover:bg-green-700 transition duration-300">
