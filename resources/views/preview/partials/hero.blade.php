...
<section id="home" class="relative min-h-[85vh] flex flex-col justify-between overflow-hidden bg-[#0B2545] text-white">
  ...
  <div class="absolute inset-0 z-0">
    ...
    <div class="absolute inset-0 bg-gradient-to-t from-[#0B2545] via-transparent to-black/20"></div>
  </div>
    ...
    <div class="max-w-3xl space-y-6">
      ...
      @if(!empty($school['hero']['badge']))
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs uppercase tracking-[0.25em] text-[#C8A951] font-sans font-semibold">
            {{ $school['hero']['badge'] }}
          </span>
        </div>
      @endif
      ...
      @if(!empty($school['hero']['subtitle']))
        <p class="text-base sm:text-lg text-[#FAF8F4]/90 max-w-[62ch] font-sans font-normal leading-[1.7]">
      ...
      <div class="pt-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
        @if(!empty($school['hero']['primary_cta_text']))
          <a href="{{ $school['hero']['primary_cta_link'] ?? '#admissions' }}"
             class="px-8 py-4 uppercase text-xs sm:text-sm tracking-wider font-sans font-semibold bg-[#C8A951] text-[#0B2545] hover:bg-[#b89840] transition text-center rounded-none shadow-none">
        ...
        @if(!empty($school['hero']['secondary_cta_text']))
          <a href="{{ $school['hero']['secondary_cta_link'] ?? '#about' }}"
             class="px-8 py-4 uppercase text-xs sm:text-sm tracking-wider font-sans font-semibold border border-white/40 text-white bg-transparent hover:bg-white hover:text-[#0B2545] transition text-center rounded-none shadow-none">
